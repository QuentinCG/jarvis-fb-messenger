var_jv_pg_fb_email="YOUR EMAIL ADDRESS HERE" # Facebook email used as username (used by Jarvis to send/receive messages/image)
var_jv_pg_fb_password="YOUR PASSWORD HERE" # Facebook password
var_jv_pg_fb_verbose="True" # Have verbose information (True/False)
var_jv_pg_fb_mute="True" # Speak with Jarvis without sound (True/False)
var_jv_pg_fb_getId="True" # Send sender ID back to user which sent a message (useful to get list of people to allow later)

# Not used yet:
#var_jv_pg_fb_allow_all_control="True" # Allow all people to control Jarvis (True/False)
#var_jv_pg_fb_allowed_people_ids="[\"\", \"\", \"\"]" # List of allowed people (facebook IDs) to control Jarvis
